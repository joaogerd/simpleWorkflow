...
def test_running_attempt_is_recovered_from_final_metadata(tmp_path: Path) -> None:
    ...
    engine.state.set_status(
        "recover", "task", "running", None, "signature", attempt_dir=str(attempt)
    )
    engine.state.reconcile_running("recover")
    assert engine.state.get_status("recover", "task") == "success"
...
def test_descendants_become_blocked_after_dependency_failure(tmp_path: Path) -> None:
    ...
    assert engine.run() == 7
    assert engine.state.get_status("blocked", "b") == "blocked"
    assert engine.state.get_status("blocked", "c") == "blocked"
